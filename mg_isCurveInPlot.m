...
function result = mg_isCurveInPlot(varargin)
    result = false();
    possibleProperties = ["Color","LineStyle","LineWidth","Marker","MarkerSize","MarkerFaceColor","XData","YData","ZData","DisplayName"];
    ...
    %% Generate struct of searched for properties from varargin
    if length(varargin) == 2
        properties = varargin;
    else
       properties = reshape(varargin, [length(varargin)/2, 2])'; 
    end
    
    %% Check if wanted properties can be checked
    stringProperties = [];
    for i = 1:size(properties,1)
        stringProperties = [stringProperties; convertCharsToStrings(properties(i,1))];
    end
